...
test("server-renders the Dragon Mode loading shell", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);

  const html = await response.text();
  assert.match(html, /<title>Dragon Mode — Protect your hoard<\/title>/i);
...
test("keeps the loading shell branded, responsive, and disposable", async () => {
  ...
  assert.match(layout, /title: "Dragon Mode — Protect your hoard"/);
  assert.match(layout, /applicationName: "Dragon Mode"/);
