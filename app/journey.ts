...
export const EVERGREEN_ACT_ONE: AuthoredChapter[] = [
  ...
  { contentId: "act1-count-treasure", chapterNumber: 2, narrativeLayer: "evergreen", replayable: true, title: "Count the Treasure", speaker: "Bramble Stoneheart", opening: "Bramble opens the first ledger and leaves room for both known figures and honest estimates. A map may begin before every road is known.", ending: "One balance is enough to wake the chamber. The original statement row stays available beside anything you import.", choices: ["Map one known balance", "Begin with an estimate", "Paste what the bank already shows"], actionTitle: "Map or import one balance", actionDescription: "Add one useful balance or check rows from a statement.", actionCategory: "Tend" },
  ...
  { contentId: "act1-star-vault", chapterNumber: 8, narrativeLayer: "evergreen", replayable: true, title: "The Star Vault", speaker: "Sol Arden", opening: "Sol separates three lights: regular interest, a temporary higher rate, and a bonus whose rules are not yet known. None is added to the tracked balance before it really arrives.", ending: "The estimate is useful because its limits are visible. A future statement may confirm or correct it.", choices: ["Check the interest rates", "Mark when the special rate ends", "Leave unknown bonuses unknown"], actionTitle: "Review one savings rate", actionDescription: "Check the regular rate, special-rate end, bonus rules, or locked-deposit end date.", actionCategory: "Learn" },
  { contentId: "act1-storm-map", chapterNumber: 9, narrativeLayer: "evergreen", replayable: true, title: "The Storm Map", speaker: "Mara Ironroot", opening: "Rain crosses the map and one tracked balance does not match the statement. Mara draws the difference in clear ink. No hidden adjustment is allowed to make the storm look prettier.", ending: "The difference remains visible, limited, and easy to revisit. Earlier victories stay exactly where they were.", choices: ["Review the difference", "Hold uncertain rows for later", "Ask for one gentler next step"], actionTitle: "Check the latest statement", actionDescription: "Review what was added or confirm the balance. Do not force a guess.", actionCategory: "Guard" },
...
function personalChronicleCopy(state: DragonState, now: Date): AuthoredChapter | null {
  ...
  if (committed && !seen.has("chronicle-first-import")) return { contentId: "chronicle-first-import", narrativeLayer: "chronicle", replayable: true, triggerId: committed.id, factsUsed: [committed.id, committed.accountId], title: "Ink That Remembers", speaker: "Bramble Stoneheart", opening: `Bramble adds ${committed.sourceDisplayName} to the archive. Every row remembers whether it was added, skipped, or left for later.`, ending: "You can still undo the import, and DragonMode remembers what changed.", choices: ["See what was saved", "Check one original row", "Carry the pages onward"], actionTitle: "Review the statement import", actionDescription: "See what was added, skipped, updated, or left for later.", actionCategory: "Guard" };
  const reconciled = state.imports.reconciliations.find((item) => item.status === "reconciled");
  if (reconciled && !seen.has("chronicle-first-reconciliation")) return { contentId: "chronicle-first-reconciliation", narrativeLayer: "chronicle", replayable: true, triggerId: reconciled.id, factsUsed: [reconciled.id, reconciled.accountId], title: "The Balanced Bell", speaker: "Bramble Stoneheart", opening: "For the first time, the statement and a checked balance ring the same note. Bramble celebrates the careful check, not the amount.", ending: "The bell becomes a permanent memory of a balance that could explain itself.", choices: ["Keep the result", "Thank the careful check", "Let the bell ring once"], actionTitle: "Keep the checked date", actionDescription: "No action is required; the successful check remains in your history.", actionCategory: "Tend" };
  ...
  if (posted && !seen.has("chronicle-pending-posted")) return { contentId: "chronicle-pending-posted", narrativeLayer: "chronicle", replayable: true, triggerId: posted.id, factsUsed: [posted.id, posted.matchedTransactionId ?? "pending"], title: "The Vanishing Hold", speaker: "Bramble Stoneheart", opening: "A temporary hold becomes a completed charge. The earlier row is remembered, but the balance counts only one movement.", ending: "What changed is visible; what was temporary is not counted twice.", choices: ["See what changed", "Keep the completed charge", "Close the hold gently"], actionTitle: "See what replaced the hold", actionDescription: "The earlier pending row is still available if you need to check it.", actionCategory: "Guard" };
  ...
  if (verifiedInterest && !seen.has("chronicle-posted-interest")) return { contentId: "chronicle-posted-interest", narrativeLayer: "chronicle", replayable: true, triggerId: verifiedInterest.id, factsUsed: [verifiedInterest.id, verifiedInterest.accountId], title: "A Star Becomes Ink", speaker: "Sol Arden", opening: "Real interest has appeared in the records. Sol places it beside the earlier estimate without pretending the estimate caused or predicted it.", ending: "Estimated light and real interest remain separate, ready to compare.", choices: ["Compare the two", "Keep the real amount", "Review the interest rate"], actionTitle: "Compare real and estimated interest", actionDescription: "Compare the rate you entered with the interest that actually arrived, without changing either.", actionCategory: "Learn" };
