    ...
    return (
      <section className="screen screen-lair lair-empty">
        ...
        <section className="empty-lair-hero">
          ...
          <div><small>Start here · private and empty</small><strong>The sky-vault is ready.</strong><p>Your Dragon {state.profile.dragonName} only needs the first balance you find useful.</p><button type="button" onClick={() => setSheet({ type: "add-account", title: "Map your first balance" })}><Plus size={18} /> Map first balance</button></div>
        </section>
        <SetupTrail state={state} navigate={navigate} setSheet={setSheet} />
        <section className="local-promise"><ShieldCheck size={25} /><div><strong>Private by default</strong><p>Your financial records stay on this device. No DragonMode sign-in, bank connection, or market feed is required.</p></div></section>
...
function IdleVaultScreen({ state, navigate, updateState, setToast }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void }) {
  ...
  return <section className="screen screen-idle">
    ...
    <section className="idle-source-notes"><SectionTitle title="How estimates work" /><p>DragonMode splits the interval at confirmed balance changes, transaction dates, rate changes, and promotion start/end dates. Unknown bonus conditions are excluded. Dividend yield remains a separate illustration because holding eligibility and distributions may differ. Posted interest is compared beside the estimate and is never duplicated or folded into it.</p><button type="button" className="secondary-button full" onClick={() => navigate("hoard")}><Landmark size={17} /> Review mapped rates</button><button type="button" className="secondary-button full" onClick={() => navigate("investments")}><Sprout size={17} /> Review investment yields</button></section>
  </section>;
}
...
function ImportScreen({ state, navigate, updateState, setToast }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void }) {
  ...
  return <section className="screen screen-import">
    ...
    {!state.accounts.length ? <div className="empty-state"><Landmark size={34} /><strong>Map a balance before bringing in rows</strong><p>DragonMode needs to know which manual register these movements belong to.</p><button type="button" onClick={() => navigate("hoard")}>Return to the Hoard</button></div> : <>
...
function WishClaim({ wish, state, updateState, setSheet, setToast, navigate }: { wish: DragonState["wishes"][number]; state: DragonState; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void; navigate: (screen: Screen) => void }) {
  ...
  const claim = () => {
    ...
    cancelWishReminder(wish.id).catch(() => undefined);
    setSheet(null);
    setToast("Purchase recorded and mapped balance reconciled · +15 XP");
    navigate("quests");
  };
...
function SettingsScreen({ state, navigate, updateState, setToast, setSheet, startTutorial }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void; setSheet: (sheet: Sheet) => void; startTutorial: () => void }) {
  ...
  return (
    <section className="screen screen-treasury screen-settings">
      ...
      <section className="privacy-panel"><ShieldCheck size={25} /><div><small>Privacy at a glance</small><strong>No sign-in. No bank connection. No market calls.</strong><p>Financial records, original imported source rows, reconciliation receipts, rules, story choices, and reveal history stay in this app&apos;s private on-device vault. A chosen file is read only after your action. Export happens only when you tap Export JSON.</p></div></section>
...
function AddAccount({ updateState, setSheet, setToast }: { updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  return <form className="form-stack" onSubmit={(event) => { event.preventDefault(); if (!name) return; const numericBalance = Number(balance) || 0; const hoardAmount = type === "credit" || type === "loan" ? 0 : Math.max(0, numericBalance); const id = crypto.randomUUID(); const capturedAt = new Date().toISOString(); updateState((previous) => ({ ...previous, accounts: [...previous.accounts, { id, name, institutionName, type, balance: numericBalance, availableBalance: type === "transaction" || type === "cash" ? numericBalance : undefined, apy: Number(apy) || undefined, compounding, includedInHoard: true, chamberId, icon: type === "investment" ? "sprout" : type === "credit" ? "card" : "vault", color: type === "investment" ? "#66c79a" : "#5b55d6", archived: false, lastConfirmedBalance: numericBalance, lastConfirmedAt: capturedAt, reconciliationStatus: "approximate" as const, balanceSnapshots: [{ id: crypto.randomUUID(), accountId: id, balance: numericBalance, capturedAt, source: "manual" as const, confirmed: true }] }], chambers: previous.chambers.map((chamber) => chamber.id === chamberId ? { ...chamber, amount: chamber.amount + hoardAmount } : chamber) })); setSheet(null); setToast("Balance mapped with a dated snapshot"); }}><label>Balance name<input required value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Everyday spending" /></label><label>Provider<input value={institutionName} onChange={(event) => setInstitutionName(event.target.value)} placeholder="Optional reference only" /></label><div className="edit-grid"><label>Current balance<input inputMode="decimal" value={balance} onChange={(event) => setBalance(event.target.value)} /></label><label>Kind<select value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="cash">Cash</option><option value="transaction">Everyday money</option><option value="savings">Savings</option><option value="credit">Credit</option><option value="loan">Loan</option><option value="investment">Investment</option><option value="asset">Asset</option></select></label><label>Deposit APY %<input inputMode="decimal" value={apy} onChange={(event) => setApy(event.target.value)} placeholder="Optional" /></label><label>Compounding<select value={compounding} onChange={(event) => setCompounding(event.target.value as typeof compounding)}><option value="daily">Daily</option><option value="monthly">Monthly</option><option value="annual">Annual</option></select></label><label>Chamber<select value={chamberId} onChange={(event) => setChamberId(event.target.value)}><option value="hearth">The Hearth</option><option value="vault">Deep Vault</option><option value="workshop">Workshop</option><option value="roost">The Roost</option><option value="sleep">Long Sleep</option><option value="tribute">Tribute Hall</option><option value="wish">Wish Vault</option></select></label></div><p className="estimate-note">This is a manual tracking record, not a bank connection. APY is used only for Idle Vault estimates until real interest is recorded as a movement.</p><button className="primary-button full" type="submit"><Plus size={18} /> Map balance</button></form>;
}
...
function GuidedTutorial({ state, screen, step, setStep, finish, navigate, setSheet }: { state: DragonState; screen: Screen; step: number; setStep: (step: number) => void; finish: () => void; navigate: (screen: Screen) => void; setSheet: (sheet: Sheet) => void }) {
  ...
  const content = [
    { eyebrow: "Welcome · begin where useful", title: "Let’s map one thing together.", body: "Choose a balance you already know or can estimate. DragonMode records what you enter; it never asks for bank credentials." },
  ...
  const primaryLabel = safeStep === 0 ? "Map my first balance" : safeStep === 1 ? (state.accounts.length ? "Continue" : screen === "hoard" ? "Map a balance" : "Open Hoard") : safeStep === 2 ? (state.accounts.length ? "Open Trusted Ledger" : "Map a balance first") : safeStep === 3 ? "Continue to review habits" : safeStep === 4 ? "Show learning tools" : safeStep === 5 ? "Finish on my Lair" : "Finish guided tour";
