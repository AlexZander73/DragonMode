...
function PlainLanguageBar({ screen }: { screen: Screen }) {
  const labels: Record<Screen, string> = {
    lair: "Home · what your money looks like right now",
    ...
    hoard: "Money · mapped balances and movements",
    import: "Statements · bring in rows, check them, and undo mistakes",
    ...
    settings: "Settings · preferences, reminders, and backups",
  };
...
function LairScreen({ state, summary, navigate, updateState, setSheet, setToast }: { state: DragonState; summary: ReturnType<typeof getHoardSummary>; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
    ...
    return (
      <section className="screen screen-lair lair-empty">
        ...
        <section className="empty-lair-hero">
          ...
          <div><small>Start here · nothing entered yet</small><strong>The sky-vault is ready.</strong><p>Your Dragon {state.profile.dragonName} only needs the first balance you find useful.</p><button type="button" onClick={() => setSheet({ type: "add-account", title: "Map your first balance" })}><Plus size={18} /> Map first balance</button></div>
        </section>
        <SetupTrail state={state} navigate={navigate} setSheet={setSheet} />
        <section className="local-promise"><ShieldCheck size={25} /><div><strong>Nothing to connect</strong><p>You type in what you want to track. DragonMode never logs in to a bank or moves your money.</p></div></section>
  ...
  return (
    <section className="screen screen-lair">
      ...
      <button className="dragon-hero lair-art" type="button" onClick={() => navigate("dragon")} aria-label={`View Your Dragon ${state.profile.dragonName}'s status`}>
        <span className="hero-shine" />
        <span className="dragon-name"><Eye size={14} /> Your Dragon {state.profile.dragonName} · Content</span>
      </button>
...
function HoardCheckPanel({ state, navigate, updateState, setSheet, setToast }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  const resolveEcho = (resolution: Parameters<typeof resolveCommittedCandidate>[3]) => {
    ...
    try {
      ...
      setToast(resolution === "not-sure" ? "The echo remains visible for later" : "Your choice was saved; the balance may need a fresh check");
    } catch (problem) {
  ...
  if (!expanded) return <button className={`hoard-check-launcher check-${check.ledgerStatus}`} type="button" onClick={() => setExpanded(true)} aria-expanded="false"><span><ShieldCheck size={18} /></span><span><small>{check.kind} Hoard Check</small><strong>{review ? "One useful review is waiting" : check.alreadyCompleted ? "All settled for today" : "A quiet check is ready when you are"}</strong></span><ChevronRight size={17} /></button>;
  return <section className={`hoard-check check-${check.ledgerStatus}`}>
    <div className="hoard-check-heading"><span><ShieldCheck size={22} /></span><div><small>{check.kind} Hoard Check</small><strong>{check.message}</strong></div><button className="hoard-check-close" type="button" onClick={() => setExpanded(false)} aria-label="Collapse Hoard Check"><ChevronRight size={18} /></button></div>
    ...
    {review && <article className="check-review"><div><small>One useful thing</small><strong>{review.title}</strong><p>{review.body}</p></div>{review.kind === "import" ? <div className="check-review-actions"><button type="button" onClick={() => resolveEcho("both-happened")}>Both happened</button><button type="button" onClick={() => resolveEcho("one-is-echo")}>One is an echo</button>{review.candidate.lifecycleRelationship === "pending-posted" && <button type="button" onClick={() => resolveEcho("pending-posted")}>Pending became posted</button>}{review.candidate.transferCandidateId && <button type="button" onClick={() => resolveEcho("confirm-transfer")}>Pair as one movement</button>}<button type="button" onClick={() => resolveEcho("not-sure")}>Not sure</button></div> : <button type="button" onClick={openReview}>Open review <ChevronRight size={15} /></button>}</article>}
  ...
  return (
    <section className="screen screen-journey">
      ...
      <section className="journey-status" style={{ "--route-color": directionMeta.color } as React.CSSProperties}>
        ...
        <div><small>Today · {directionMeta.label}</small><strong>{chapter?.title ?? "A quiet page"}</strong><p>{directionMeta.body}</p><span><DirectionIcon size={16} /> {latestSnapshot ? `${latestSnapshot.change >= 0 ? "+" : "−"}${formatGold(Math.abs(latestSnapshot.change), 0)} since your last check` : "First check recorded"}</span></div>
      </section>
      <section className="atlas-map" aria-label="Living Atlas level map">
        ...
        {JOURNEY_NODE_POSITIONS.map((position, index) => {
          ...
          return <button
            ...
            onClick={() => nodeChapter ? setSheet({ type: "journey-story", id: nodeChapter.id, title: nodeChapter.title }) : setSheet({ type: "story", title: current ? "Today’s point" : "A future path", body: current ? "Today’s numbers placed the party here. Open the chapter below to choose how to travel onward." : "This point opens with time and check-ins—not with a required balance." })}
...
function IdleVaultScreen({ state, navigate, updateState, setToast }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void }) {
  ...
  return <section className="screen screen-idle">
    ...
    <section className="idle-source-notes"><SectionTitle title="How estimates work" /><p>DragonMode estimates each day using the balance and interest rate entered for that date. It stops a special rate on its end date and leaves out bonuses when the rules are unknown. Dividend estimates stay separate because eligibility and payment dates vary. Real interest you record is shown beside the estimate and never counted twice.</p><button type="button" className="secondary-button full" onClick={() => navigate("hoard")}><Landmark size={17} /> Review interest rates</button><button type="button" className="secondary-button full" onClick={() => navigate("investments")}><Sprout size={17} /> Review dividend estimates</button></section>
  </section>;
}
...
function HoardScreen({ state, summary, navigate, setSheet }: { state: DragonState; summary: ReturnType<typeof getHoardSummary>; navigate: (screen: Screen) => void; setSheet: (sheet: Sheet) => void }) {
  const [view, setView] = useState<"Chambers" | "Balances" | "Transactions">("Chambers");
  const [query, setQuery] = useState("");
  const [categoryFilter, setCategoryFilter] = useState("All chambers");
  const [accountFilter, setAccountFilter] = useState("All balances");
  ...
  const filteredTransactions = searchTransactions(state.transactions, query).filter((transaction) => {
    ...
    const accountMatches = accountFilter === "All balances" || transaction.accountId === accountFilter;
  ...
  return (
    <section className={`screen screen-hoard hoard-view-${view.toLowerCase()}`}>
      ...
      <section className="hoard-total">
        ...
        <span>Total hoard value</span><strong>{formatGold(summary.total)}</strong><small>{state.accounts.length} mapped {state.accounts.length === 1 ? "balance" : "balances"}</small>
      </section>
      <Segmented options={["Chambers", "Balances", "Transactions"]} value={view} onChange={(value) => setView(value as typeof view)} />
      {view === "Chambers" && (
          ...
          <div className="cream-list">
            {state.accounts.filter((account) => !account.archived).map((account) => {
              ...
              return <button key={account.id} type="button" onClick={() => setSheet({ type: "account", id: account.id, title: account.name })}>
                <span className={`round-icon ${account.type}`} style={{ "--account": account.color } as React.CSSProperties}><AccountIcon size={20} /></span><span><strong>{account.name}</strong><small>{account.institutionName || account.type} · {account.reconciliationStatus === "reconciled" ? "Checked" : account.reconciliationStatus === "needs-review" ? "Needs a look" : account.reconciliationStatus === "stale" ? "May be out of date" : "Estimated"}{account.importedThrough ? ` through ${formatDate(account.importedThrough)}` : ""}</small></span><b>{formatGold(account.balance)}</b><ChevronRight size={17} />
              </button>;
            })}
      ...
      {view === "Transactions" && (
        ...
        <div className="transaction-panel">
          ...
          <div className="transaction-filters">
            ...
            <select value={accountFilter} onChange={(event) => setAccountFilter(event.target.value)} aria-label="Filter by mapped balance"><option>All balances</option>{state.accounts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select>
...
function ImportScreen({ state, navigate, updateState, setToast }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void }) {
  ...
  const saveMappingTemplate = () => {
    ...
    updateState((previous) => ({ ...previous, imports: { ...previous.imports, mappingTemplates: [{ id, name: sourceName === "Pasted transactions" ? `Remembered format ${previous.imports.mappingTemplates.length + 1}` : sourceName, delimiter, dateOrder, signConvention, columns: { date: "Date", description: "Description", amount: "Amount", debit: "Debit", credit: "Credit", status: "Status", sourceId: "Transaction ID" } }, ...previous.imports.mappingTemplates] } }));
    setTemplateId(id);
  ...
  return <section className="screen screen-import">
    <ScreenHeader icon={Upload} title="Statement helper" subtitle="Paste, check, save, or undo" back={() => navigate("hoard")} />
    ...
    {!state.accounts.length ? <div className="empty-state"><Landmark size={34} /><strong>Map a balance before bringing in rows</strong><p>Choose which balance these movements belong to.</p><button type="button" onClick={() => navigate("hoard")}>Return to the Hoard</button></div> : <>
      <section className="import-source-card">
        <label>Mapped balance<select value={accountId} onChange={(event) => { setAccountId(event.target.value); setStaged(null); }}>{state.accounts.filter((account) => !account.archived).map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>
        {state.imports.mappingTemplates.length > 0 && <label>Remembered format<select value={templateId} onChange={(event) => { const id = event.target.value; setTemplateId(id); const template = state.imports.mappingTemplates.find((item) => item.id === id); if (template) { setDateOrder(template.dateOrder); setSignConvention(template.signConvention); } setStaged(null); }}><option value="">Work it out for me</option>{state.imports.mappingTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label>}
        ...
        <label>Paste statement rows<textarea value={source} onChange={(event) => { setSource(event.target.value); setSourceName("Pasted transactions"); setSourceKind("paste"); setStaged(null); }} placeholder={"Date,Description,Amount,Transaction ID\n21/07/2026,Hearth Market,-45.20,abc-123"} /></label>
        <div className="import-actions"><label className="secondary-button"><Upload size={17} /> Choose a statement file<input type="file" accept=".csv,.txt,.ofx,.qfx,.qif,text/plain,text/csv" onChange={(event) => loadFile(event.target.files?.[0])} /></label><button type="button" className="primary-button" disabled={!source.trim() || !accountId} onClick={preview}><Eye size={17} /> Check these rows</button></div>
        <p className="fine-print">DragonMode keeps the text you brought in so you can check or undo it. It never asks for your bank sign-in details.</p>
      </section>
      {error && <p className="import-error" role="alert">{error}</p>}
      {staged && <section className="import-preview">
        <div className="import-preview-heading"><div><small>{staged.sourceDisplayName}</small><strong>{staged.candidates.length} checked {staged.candidates.length === 1 ? "row" : "rows"}</strong><p>{safeCount} ready · {skippedCount} already handled · {unresolvedCount} need your help</p><button type="button" className="inline-link" onClick={saveMappingTemplate}>Remember this format</button></div><span>{Math.round(staged.candidates.reduce((sum, candidate) => sum + candidate.confidence, 0) / staged.candidates.length * 100)}% clear</span></div>
        {staged.ambiguityWarnings.length > 0 && <div className="import-error" role="alert"><strong>Please check how these rows were read before saving them.</strong> {staged.ambiguityWarnings.join(" ")}</div>}
        <div className="candidate-list">{staged.candidates.map((candidate) => {
          ...
          return <article key={candidate.id} className={`${held ? "candidate-held" : "candidate-safe"} ${resolved ? "candidate-resolved" : ""}`}>
            ...
            {candidate.proposedAction === "skip-exact" && <p className="candidate-status safe"><Check size={15} /> This exact row was already added, so it will not be added again.</p>}
            {held && <div className="candidate-decision"><p><strong>DragonMode noticed a possible relationship.</strong> {candidate.duplicateReasons.join(" ")}</p><div>
              ...
              {candidate.transferCandidateId && <button type="button" className={candidate.resolution === "confirm-transfer" ? "selected" : ""} onClick={() => resolve(candidate.id, "confirm-transfer")}>Pair as one movement</button>}
            ...
            <details><summary>Show the original row</summary><code>{candidate.rawSourceRow}</code></details>
          </article>;
        })}</div>
        ...
        <section className="reconciliation-entry"><div><strong>Optional final balance check</strong><p>Enter the balance shown on the statement to see whether the rows you chose add up.</p></div><label>Closing balance<input inputMode="decimal" value={closingBalance} onChange={(event) => setClosingBalance(event.target.value)} placeholder="Leave blank if unknown" /></label><label>Statement through<input type="date" value={periodEnd} onChange={(event) => setPeriodEnd(event.target.value)} /></label></section>
        <button type="button" className="primary-button full" onClick={commit}><ShieldCheck size={18} /> Save checked rows</button>
      </section>}
      <SectionTitle title="Past statement imports" />
      {state.imports.batches.length ? <div className="receipt-list">{state.imports.batches.slice(0, 10).map((batch) => {
        ...
        return <article key={batch.id} className={`receipt-${batch.status}`}><span><Upload size={19} /></span><div><small>{formatDate(batch.importedAt ?? batch.createdAt)} · {batch.sourceDisplayName}</small><strong>{batch.receiptNote ?? `${batch.candidates.length} statement rows`}</strong><p>{reconciliation ? `${reconciliation.status === "reconciled" ? "Matches" : "Needs another look"} · difference ${formatGold(Math.abs(reconciliation.difference))}` : "No closing balance supplied"}</p></div>{batch.status === "committed" && batch.undoSnapshot ? <button type="button" onClick={() => undo(batch.id)}>Undo</button> : <em>{batch.status === "undone" ? "Undone" : "Saved"}</em>}</article>;
      })}</div> : <div className="empty-state compact"><Clock size={28} /><strong>No statement imports yet</strong><p>After you save an import, you can review or undo it here.</p></div>}
    </>}
  </section>;
}
...
function DragonScreen({ state, navigate, updateState, setToast }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void }) {
  ...
  return (
    <section className="screen screen-dragon">
      <ScreenHeader icon={Eye} title={`Your Dragon ${state.profile.dragonName}`} subtitle="A companion to your stewardship" back={() => navigate("lair")} />
      <section className={`dragon-status-art skin-preview lair-${previewTheme.toLowerCase().replace(/[^a-z0-9]+/g, "-")} dragon-${previewColor.toLowerCase()}`}><span>Your Dragon {state.profile.dragonName} · {previewColor} in {previewTheme}</span></section>
...
function SettingsScreen({ state, navigate, updateState, setToast, setSheet, startTutorial }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void; setSheet: (sheet: Sheet) => void; startTutorial: () => void }) {
  ...
  const permissionLabel = ({ checking: "Checking…", web: "Available in the installed app", granted: "Allowed", denied: "Not allowed", unavailable: "Not available here" } as Record<string, string>)[permissionState] ?? "Ready";
  ...
  const notificationOptions: Array<{ key: keyof DragonState["profile"]["notificationPreferences"]; label: string }> = [
    ...
    { key: "importReview", label: "Statement rows that need your help" },
    { key: "importantUncertainty", label: "Something important to double-check" },
    { key: "expectedIncome", label: "Expected income reminder" },
    { key: "rateOrMaturity", label: "Savings rate or end-date reminder" },
    { key: "storyChapter", label: "A new story chapter" },
  ];
  return (
    <section className="screen screen-treasury screen-settings">
      ...
      <section className="keeper-desk" aria-label="Keeper's desk">
        ...
        <small>Choose what helps, leave the rest, and change your mind at any time.</small>
      </section>
      <section className="settings-card"><div><span><Volume2 size={20} /><b>Sound effects</b></span><button type="button" role="switch" aria-label="Sound effects" aria-checked={state.profile.soundEnabled} className={state.profile.soundEnabled ? "toggle on" : "toggle"} onClick={() => { updateState((previous) => ({ ...previous, profile: { ...previous.profile, soundEnabled: !previous.profile.soundEnabled } })); playFeedback({ sound: !state.profile.soundEnabled, haptics: false }).catch(() => undefined); }}><i /></button></div><div><span><Zap size={20} /><b>Haptic feedback</b></span><button type="button" role="switch" aria-label="Haptic feedback" aria-checked={state.profile.hapticsEnabled} className={state.profile.hapticsEnabled ? "toggle on" : "toggle"} onClick={() => { updateState((previous) => ({ ...previous, profile: { ...previous.profile, hapticsEnabled: !previous.profile.hapticsEnabled } })); playFeedback({ sound: false, haptics: !state.profile.hapticsEnabled }).catch(() => undefined); }}><i /></button></div><div><span><Bell size={20} /><b>Allow reminders</b></span><button type="button" role="switch" aria-label="Allow reminders" aria-checked={state.profile.notificationsEnabled} className={state.profile.notificationsEnabled ? "toggle on" : "toggle"} onClick={() => updateState((previous) => ({ ...previous, profile: { ...previous.profile, notificationsEnabled: !previous.profile.notificationsEnabled } }))}><i /></button></div><div><span><Sparkles size={20} /><b>Reduced motion</b></span><button type="button" role="switch" aria-label="Reduced motion" aria-checked={state.profile.reducedMotion} className={state.profile.reducedMotion ? "toggle on" : "toggle"} onClick={() => updateState((previous) => ({ ...previous, profile: { ...previous.profile, reducedMotion: !previous.profile.reducedMotion } }))}><i /></button></div><div><span><BookOpen size={20} /><b>Helpful hints</b></span><button type="button" role="switch" aria-label="Helpful hints" aria-checked={state.profile.plainLanguage} className={state.profile.plainLanguage ? "toggle on" : "toggle"} onClick={() => updateState((previous) => ({ ...previous, profile: { ...previous.profile, plainLanguage: !previous.profile.plainLanguage } }))}><i /></button></div></section>
      <section className="preference-grid"><SectionTitle title="Display and currency" /><label>Currency<select value={state.profile.preferredCurrency} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, preferredCurrency: event.target.value } }))}><option value="AUD">AUD · Australian dollar</option><option value="USD">USD · US dollar</option><option value="NZD">NZD · New Zealand dollar</option><option value="GBP">GBP · British pound</option><option value="EUR">EUR · Euro</option><option value="CAD">CAD · Canadian dollar</option></select></label><label>Date and number format<select value={state.profile.locale} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, locale: event.target.value } }))}><option value="en-AU">English (Australia)</option><option value="en-NZ">English (New Zealand)</option><option value="en-US">English (United States)</option><option value="en-GB">English (United Kingdom)</option></select></label><label>Text size <b>{Math.round(state.profile.fontScale * 100)}%</b><input type="range" min="0.9" max="1.35" step="0.05" value={state.profile.fontScale} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, fontScale: Number(event.target.value) } }))} /></label></section>
      <section className="notification-settings"><div className="notification-heading"><SectionTitle title="Reminders" /><span>{permissionLabel}</span></div><p>Choose only the reminders that help. DragonMode will not nudge you just to keep a streak going, and reminders never show money details.</p><div className="notification-list">{notificationOptions.map(({ key, label }) => <label className="notification-choice" key={key}><input type="checkbox" checked={state.profile.notificationPreferences[key]} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, notificationPreferences: { ...previous.profile.notificationPreferences, [key]: event.target.checked } } }))} /><span>{label}</span></label>)}</div><div className="notification-rhythm"><label>Do not disturb from<input type="time" value={state.profile.notificationQuietStart} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, notificationQuietStart: event.target.value } }))} /></label><label>Until<input type="time" value={state.profile.notificationQuietEnd} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, notificationQuietEnd: event.target.value } }))} /></label><label>Weekly check-in day<select value={state.profile.reviewDay} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, reviewDay: Number(event.target.value) } }))}><option value="1">Monday</option><option value="2">Tuesday</option><option value="3">Wednesday</option><option value="4">Thursday</option><option value="5">Friday</option><option value="6">Saturday</option><option value="7">Sunday</option></select></label><label>Check-in time<select value={state.profile.reviewHour} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, reviewHour: Number(event.target.value) } }))}>{[8, 10, 12, 14, 16, 18, 20].map((hour) => <option key={hour} value={hour}>{new Date(2000, 0, 1, hour).toLocaleTimeString(state.profile.locale, { hour: "numeric", minute: "2-digit" })}</option>)}</select></label></div></section>
      <section className="budget-settings"><SectionTitle title="Planning preferences" /><label>Protected minimum buffer <b>{formatGold(state.profile.minimumBuffer, 0)}</b><input type="range" min="0" max="5000" step="100" value={state.profile.minimumBuffer} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, minimumBuffer: Number(event.target.value) } }))} /></label><label>Comfortable monthly cost <b>{formatGold(state.profile.comfortableMonthlyCost, 0)}</b><input type="range" min="0" max="6000" step="50" value={state.profile.comfortableMonthlyCost} onChange={(event) => updateState((previous) => ({ ...previous, profile: { ...previous.profile, comfortableMonthlyCost: Number(event.target.value) } }))} /></label></section>
      <section className="data-panel"><h2>Backups and reset</h2><p>Save a backup before changing phones or deleting the app. It includes everything you entered, plus your story and collection progress.</p><button type="button" onClick={exportData}><Download size={18} /> Save a backup</button><label><Upload size={18} /> Restore from a backup<input type="file" accept="application/json" onChange={(event) => importData(event.target.files?.[0])} /></label><button type="button" onClick={() => setSheet({ type: "empty-vault", title: "Start fresh at zero" })}><Sparkles size={18} /> Start fresh at zero</button></section>
      ...
      <p className="version-note">{APP_NAME} 1.0</p>
    </section>
  );
}
  ...
  return (
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setSheet(null); }}>
      <section ref={dialogRef} className={`modal-sheet ${sheet.type === "story-scene" || sheet.type === "journey-story" ? "story-modal" : ""}`} role="dialog" aria-modal="true" aria-labelledby="modal-title">
        ...
        {sheet.type === "transaction" && transaction?.origin === "import" && <details className="transaction-provenance"><summary><ShieldCheck size={15} /> Where this came from</summary><p><b>Original description</b>{transaction.originalDescription ?? transaction.merchant}</p><p><b>Original statement row</b><code>{transaction.rawSourceRow ?? "Original row unavailable"}</code></p></details>}
        ...
        {sheet.type === "empty-vault" && <div className="confirm-panel"><p>This erases everything you entered in DragonMode and starts again at zero. Save a backup first if you may want it later.</p><button type="button" className="danger-button" onClick={() => { clearState().then(() => { const empty = createEmptyState(); updateState(() => ({ ...empty, profile: { ...empty.profile, tutorialComplete: true, onboardingComplete: true } })); setSheet(null); navigate("lair"); setToast("DragonMode is ready for a fresh start"); }); }}>Erase and start over</button><button type="button" className="secondary-button" onClick={() => setSheet(null)}>Go back</button></div>}
...
function MetricDetail({ id, state }: { id: string; state: DragonState }) {
  ...
  const content: Record<string, { value: number; text: string }> = {
    available: { value: summary.available, text: "Liquid money visible now in your mapped everyday balances." },
...
function DebtPayment({ debt, state, updateState, setSheet, setToast }: { debt: DragonState["debts"][number]; state: DragonState; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  const record = () => {
    ...
    setSheet(null);
    setToast(`${formatGold(numericAmount, 0)} payment recorded · +5 XP`);
  };
  return <form className="form-stack" onSubmit={(event) => { event.preventDefault(); record(); }}><div className="formula-card"><LockKeyhole size={28} /><strong>{formatGold(debt.balance)} remaining</strong><p>This adds an expense and lowers the tracked debt by the same amount.</p></div><div className="edit-grid"><label>Payment amount<input required inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></label><label>Date<input type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label>Paid from<select required value={accountId} onChange={(event) => setAccountId(event.target.value)}>{eligibleAccounts.map((account) => <option value={account.id} key={account.id}>{account.name}</option>)}</select></label></div>{!eligibleAccounts.length && <p className="warning-copy">Map a cash, everyday, savings, investment, or asset balance before recording a payment.</p>}<button disabled={!eligibleAccounts.length} className="primary-button full" type="submit"><Coins size={18} /> Record payment</button></form>;
}
...
function AccountDetail({ account, state, updateState, setSheet, setToast }: { account: DragonState["accounts"][number]; state: DragonState; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  const saveAccount = () => {
    ...
    setSheet(null);
    setToast("Balance and yield settings updated");
  };
...
function AddAccount({ updateState, setSheet, setToast }: { updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  return <form className="form-stack" onSubmit={(event) => { event.preventDefault(); if (!name) return; const numericBalance = Number(balance) || 0; const hoardAmount = type === "credit" || type === "loan" ? 0 : Math.max(0, numericBalance); const id = crypto.randomUUID(); const capturedAt = new Date().toISOString(); updateState((previous) => ({ ...previous, accounts: [...previous.accounts, { id, name, institutionName, type, balance: numericBalance, availableBalance: type === "transaction" || type === "cash" ? numericBalance : undefined, apy: Number(apy) || undefined, compounding, includedInHoard: true, chamberId, icon: type === "investment" ? "sprout" : type === "credit" ? "card" : "vault", color: type === "investment" ? "#66c79a" : "#5b55d6", archived: false, lastConfirmedBalance: numericBalance, lastConfirmedAt: capturedAt, reconciliationStatus: "approximate" as const, balanceSnapshots: [{ id: crypto.randomUUID(), accountId: id, balance: numericBalance, capturedAt, source: "manual" as const, confirmed: true }] }], chambers: previous.chambers.map((chamber) => chamber.id === chamberId ? { ...chamber, amount: chamber.amount + hoardAmount } : chamber) })); setSheet(null); setToast("Balance added"); }}><label>Balance name<input required value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Everyday spending" /></label><label>Where it is held<input value={institutionName} onChange={(event) => setInstitutionName(event.target.value)} placeholder="Optional" /></label><div className="edit-grid"><label>Current balance<input inputMode="decimal" value={balance} onChange={(event) => setBalance(event.target.value)} /></label><label>Kind<select value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="cash">Cash</option><option value="transaction">Everyday money</option><option value="savings">Savings</option><option value="credit">Credit</option><option value="loan">Loan</option><option value="investment">Investment</option><option value="asset">Asset</option></select></label><label>Savings interest rate (APY) %<input inputMode="decimal" value={apy} onChange={(event) => setApy(event.target.value)} placeholder="Optional" /></label><label>Interest added<select value={compounding} onChange={(event) => setCompounding(event.target.value as typeof compounding)}><option value="daily">Daily</option><option value="monthly">Monthly</option><option value="annual">Yearly</option></select></label><label>Chamber<select value={chamberId} onChange={(event) => setChamberId(event.target.value)}><option value="hearth">The Hearth</option><option value="vault">Deep Vault</option><option value="workshop">Workshop</option><option value="roost">The Roost</option><option value="sleep">Long Sleep</option><option value="tribute">Tribute Hall</option><option value="wish">Wish Vault</option></select></label></div><p className="estimate-note">You enter this balance yourself. The interest rate is used only to estimate possible interest; DragonMode never logs in to a bank or changes real money.</p><button className="primary-button full" type="submit"><Plus size={18} /> Map balance</button></form>;
}
...
function SetupTrail({ state, navigate, setSheet }: { state: DragonState; navigate: (screen: Screen) => void; setSheet: (sheet: Sheet) => void }) {
  const tasks = [
    { title: "Map one balance", detail: "Record one useful number—an estimate is fine.", done: state.accounts.length > 0, icon: Landmark, action: () => setSheet({ type: "add-account", title: "Map a balance" }) },
    { title: "Record one movement", detail: "Income or spending helps wake the Scrying Pool.", done: state.transactions.length > 0, icon: Coins, action: () => state.accounts.length ? setSheet({ type: "add-transaction", title: "Add treasure movement" }) : setSheet({ type: "add-account", title: "Map a balance first" }) },
    ...
    { title: "Name a claimant", detail: "Optional: map one subscription or recurring cost.", done: state.subscriptions.length > 0, icon: ScrollText, action: () => state.accounts.length ? setSheet({ type: "add-subscription", title: "Add a claimant" }) : setSheet({ type: "add-account", title: "Map a balance first" }) },
  ];
...
function GuidedTutorial({ state, screen, step, setStep, finish, navigate, setSheet }: { state: DragonState; screen: Screen; step: number; setStep: (step: number) => void; finish: () => void; navigate: (screen: Screen) => void; setSheet: (sheet: Sheet) => void }) {
  ...
  const content = [
    { eyebrow: "Welcome · begin where useful", title: "Let’s map one thing together.", body: "Choose a balance you already know or can estimate. You type it in yourself—DragonMode never logs in to a bank." },
    { eyebrow: "Live step · Hoard", title: "Map a balance you want to track.", body: "Give it a familiar name, choose its kind, and enter the latest amount. You can revise or remove it whenever you need." },
    { eyebrow: "Live step · Capture", title: "Choose the quickest way to add movement.", body: "Add one item manually, paste statement rows, or choose an export file. Start with whichever takes the least effort." },
    { eyebrow: "Live step · Review", title: "Keep the interpretation honest.", body: "Trusted Ledger keeps each original row visible. When two charges look alike, you confirm whether both happened or one is an echo." },
    { eyebrow: "Live step · Review habit", title: "Check in only when it helps.", body: "The compact Hoard Check waits quietly and surfaces at most one useful uncertainty. Skipping it costs you nothing." },
    { eyebrow: "Live step · Learning", title: "Explore an answer with your own numbers.", body: "Lore calculators fill what they can and show every assumption. Change any input to compare an educational illustration—not financial advice." },
    { eyebrow: "Tour complete", title: "Choose your own rhythm.", body: "Your Lair, story, and tools are ready when useful. Replay or skip this tour from Settings without changing your records or rewards." },
  ][safeStep];
  const primary = () => {
    ...
    if (safeStep === 1) {
      ...
      return setSheet({ type: "add-account", title: "Map your first balance" });
    }
    if (safeStep === 2) {
  ...
  const primaryLabel = safeStep === 0 ? "Map my first balance" : safeStep === 1 ? (state.accounts.length ? "Continue" : screen === "hoard" ? "Map a balance" : "Open Hoard") : safeStep === 2 ? (state.accounts.length ? "Open statement helper" : "Map a balance first") : safeStep === 3 ? "Continue to review habits" : safeStep === 4 ? "Show learning tools" : safeStep === 5 ? "Finish on my Lair" : "Finish guided tour";
