...
import { createEmptyState, createSeedState, JOURNEY_AVATARS, normalizeState, type DragonState, type Goal, type IncomeCadence, type IncomeKind, type InvestmentPosition, type Pet, type Quest, type Subscription, type WorthRating } from "./data";
...
type Screen = "lair" | "pets" | "journey" | "roster" | "idle" | "hoard" | "quests" | "goals" | "analytics" | "tribute" | "flight" | "wish" | "dragon" | "debt" | "investments" | "legacy" | "settings";
  ...
  const finishTutorial = (choices: OnboardingChoices) => {
    updateState((previous) => {
      ...
      return processJourneySession({
        ...
        progression: firstRun ? base.progression : addProgressionXp(base, 10, "awakening-replayed"),
      });
    });
    setTutorialStep(0);
  };
...
function GoalsScreen({ state, navigate, updateState, setSheet, setToast }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  const addStep = (goal: Goal) => {
    ...
    updateState((previous) => ({
      ...previous,
      goals: previous.goals.map((item) => item.id === goal.id ? { ...item, currentAmount: Math.min(item.targetAmount, item.currentAmount + step), status: item.currentAmount + step >= item.targetAmount ? "completed" as const : item.status } : item),
      progression: addProgressionXp(previous, 3, `goal-step-${goal.id}-${new Date().toISOString().slice(0, 10)}`),
    }));
    setToast(`${formatGold(step, 0)} mapped toward ${goal.name} · +3 XP`);
  };
  return (
...
function SettingsScreen({ state, navigate, updateState, setToast, setSheet }: { state: DragonState; navigate: (screen: Screen) => void; updateState: (updater: (state: DragonState) => DragonState) => void; setToast: (toast: string) => void; setSheet: (sheet: Sheet) => void }) {
  ...
  return (
    <section className="screen screen-settings">
      ...
      <section className="privacy-panel"><ShieldCheck size={25} /><div><small>Privacy at a glance</small><strong>No account. No bank login. No market calls.</strong><p>Balances, transactions, goals, debts, wishes, and story progress are stored in this app&apos;s private on-device vault. Export happens only when you tap Export JSON. Import reads only the file you choose.</p></div></section>
      <section className="data-panel"><h2>Local-first records</h2><p>You control this device copy. Deleting the app can remove it, so export a backup before changing devices. Notifications are optional and can be disabled in iOS Settings at any time.</p><button type="button" onClick={exportData}><Download size={18} /> Export JSON</button><label><Upload size={18} /> Import JSON<input type="file" accept="application/json" onChange={(event) => importData(event.target.files?.[0])} /></label><button type="button" onClick={() => setSheet({ type: "reset", title: "Restore the demo hoard" })}><RotateCcw size={18} /> Restore demo</button><button type="button" onClick={() => setSheet({ type: "empty-vault", title: "Start a private empty vault" })}><Sparkles size={18} /> Start empty</button></section>
...
function GoalDetail({ goal, state, updateState, setSheet, setToast }: { goal: Goal; state: DragonState; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  return <div className="detail-stack"><div className="goal-detail-banner"><Target size={26} /><span><small>Progress is never erased</small><strong>{Math.round(goal.currentAmount / Math.max(1, goal.targetAmount) * 100)}% of this path is mapped</strong></span></div><div className="edit-grid"><label>Goal name<input value={name} onChange={(event) => setName(event.target.value)} /></label><label>Target amount<input inputMode="decimal" value={targetAmount} onChange={(event) => setTargetAmount(event.target.value)} /></label><label>Protected so far<input inputMode="decimal" value={currentAmount} onChange={(event) => setCurrentAmount(event.target.value)} /></label><label>Target date<input type="date" value={targetDate} onChange={(event) => setTargetDate(event.target.value)} /></label><label>Chamber<select value={chamberId} onChange={(event) => setChamberId(event.target.value)}>{state.chambers.map((chamber) => <option key={chamber.id} value={chamber.id}>{chamber.name}</option>)}</select></label><label>Rhythm<select value={priority} onChange={(event) => setPriority(event.target.value as Goal["priority"])}><option value="gentle">Gentle</option><option value="steady">Steady</option><option value="focused">Focused</option></select></label><label>Status<select value={status} onChange={(event) => setStatus(event.target.value as Goal["status"])}><option value="active">Active</option><option value="paused">Paused</option><option value="completed">Completed</option></select></label></div><label>Why it matters<textarea value={note} onChange={(event) => setNote(event.target.value)} /></label><button className="primary-button full" type="button" onClick={() => { const target = Math.max(1, Number(targetAmount) || 0); const current = Math.max(0, Number(currentAmount) || 0); updateState((previous) => ({ ...previous, goals: previous.goals.map((item) => item.id === goal.id ? { ...item, name: name.trim() || item.name, targetAmount: target, currentAmount: current, targetDate: new Date(`${targetDate}T12:00:00`).toISOString(), chamberId, priority, status: current >= target ? "completed" : status, note: note.trim() || "A protected goal for the road ahead." } : item), progression: current >= target && goal.status !== "completed" ? addProgressionXp(previous, 25, `goal-complete-${goal.id}`) : previous.progression })); setSheet(null); setToast(current >= target ? "Goal protected · a permanent victory" : "Goal path updated"); }}><Save size={17} /> Save goal</button><button className="danger-button full" type="button" onClick={() => { updateState((previous) => ({ ...previous, goals: previous.goals.filter((item) => item.id !== goal.id) })); setSheet(null); setToast("Goal removed; earned progress remains"); }}><Trash2 size={17} /> Remove goal</button></div>;
}
...
function AddTransaction({ state, updateState, setSheet, setToast }: { state: DragonState; updateState: (updater: (state: DragonState) => DragonState) => void; setSheet: (sheet: Sheet) => void; setToast: (toast: string) => void }) {
  ...
  const submit = () => {
    ...
    updateState((previous) => {
      const linkedAccount = previous.accounts.find((account) => account.id === accountId);
      const movement = status === "cleared" && !transfer ? (direction === "income" ? numericAmount : -numericAmount) : 0;
      return {
        ...previous,
        ...
        chambers: previous.chambers.map((chamber) => chamber.id === linkedAccount?.chamberId ? { ...chamber, amount: Math.max(0, chamber.amount + movement) } : chamber),
...
function Tutorial({ state, firstRun, step, setStep, finish }: { state: DragonState; firstRun: boolean; step: number; setStep: (step: number) => void; finish: (choices: OnboardingChoices) => void }) {
  ...
  return <div className="tutorial-backdrop"><section className={`tutorial-card onboarding-step-${safeStep}`} role="dialog" aria-modal="true" aria-labelledby="tutorial-title">
    ...
    <div className="onboarding-stage"><span className="onboarding-dragon" aria-hidden="true" />{safeStep >= 3 && <span className="onboarding-avatar" style={{ "--onboarding-avatar": `url("${selectedAvatar.asset}")` } as React.CSSProperties} aria-hidden="true" />}<div><small>{safeStep === 0 ? "Chapter 1 · The Awakening" : safeStep === 1 ? "Choose your names" : safeStep === 2 ? "Shape the sky-vault" : safeStep === 3 ? "Choose your travelling keeper" : "The keeper's promise"}</small><strong>{safeStep === 0 ? "A guardian stirs" : safeStep === 1 ? "What shall the vault call you?" : safeStep === 2 ? "Make the lair feel like yours" : safeStep === 3 ? "The Atlas follows your rhythm" : "Private. Optional. Never punishing."}</strong></div></div>
    <div className="onboarding-copy">
      ...
      {safeStep === 3 && <><h2 id="tutorial-title">Who travels the Living Atlas?</h2><p>Story Mode is optional. Financial direction changes the route and supportive story, but never removes progress.</p><div className="onboarding-roster">{JOURNEY_AVATARS.map((avatar) => <button type="button" key={avatar.id} className={choices.avatarId === avatar.id ? "selected" : ""} onClick={() => setChoices((current) => ({ ...current, avatarId: avatar.id }))}><span style={{ "--avatar-image": `url("${avatar.asset}")` } as React.CSSProperties} /><small>{avatar.name.split(" ")[0]}</small></button>)}</div><div className="journey-onboarding-controls"><label className="check-label"><input type="checkbox" checked={choices.journeyEnabled} onChange={(event) => setChoices((current) => ({ ...current, journeyEnabled: event.target.checked }))} /> Enable optional Journey chapters</label><label>Story rhythm<select value={choices.cadence} onChange={(event) => setChoices((current) => ({ ...current, cadence: event.target.value as OnboardingChoices["cadence"] }))}><option value="daily">Daily check-in</option><option value="weekly">Weekly reflection</option><option value="pay-cycle">Pay-cycle reflection</option></select></label></div></>}
      {safeStep === 4 && <><h2 id="tutorial-title">The vault is yours.</h2><p>Release mode stores your records in this app&apos;s private on-device vault. It does not require an account, bank login, or market-data connection.</p><div className="privacy-seals"><div><ShieldCheck size={20} /><span><strong>On device</strong><small>Balances, goals, debts, and story progress</small></span></div><div><Download size={20} /><span><strong>Portable</strong><small>Export or import JSON only when you choose</small></span></div><div><Bell size={20} /><span><strong>Quiet by default</strong><small>Notifications stay off until you enable them</small></span></div></div><aside><Heart size={18} /> If the path slides backward, the story adapts with hope and one actionable next step. It never gives up on you.</aside></>}
    </div>
    <div className="story-dots" aria-label={`Chapter ${safeStep + 1} of 5`}>{Array.from({ length: 5 }, (_, index) => <i key={index} className={index === safeStep ? "active" : ""} />)}</div>
    <div className="onboarding-actions">{safeStep > 0 && <button className="secondary-button" type="button" onClick={() => setStep(safeStep - 1)}><ArrowLeft size={17} /> Back</button>}<button className="primary-button" type="button" onClick={next}>{safeStep === 4 ? (choices.dataMode === "personal" && firstRun ? "Open my empty vault" : "Enter the Lair") : "Continue"} <ChevronRight size={18} /></button></div>
  </section></div>;
}
