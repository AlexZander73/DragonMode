...
export function buildCalculatorResults(state: DragonState, now = new Date()): CalculatorResult[] {
  ...
  const sourceDate = state.updatedAt ? new Date(state.updatedAt).toLocaleDateString(state.profile.locale) : "the information shown now";
  ...
  return [
    {
      ...
      fields: [{ label: "Available", value: summary.available, source: `Balances · ${sourceDate}`, editable: true }, { label: "Expected steady income", value: fortnightIncome, source: "Income routes", editable: true }, { label: "Subscriptions due", value: upcomingSubscriptions, source: "Claimants", editable: true }, { label: "Debt minimums due", value: upcomingDebt, source: "Debt map", editable: true }],
    ...
    {
      ...common, id: "promotion-cliff", fantasyTitle: "Star Vault Rate Watch", plainTitle: "Savings interest and special-rate end", question: "What might this savings balance earn before its rate changes?", result: primarySavings ? `${money(state, nextMonthInterest)} estimated over 30 days at ${savingsRate.toFixed(2)}%` : "Add a savings rate to create this illustration",
      explanation: "Balance × annual rate × 30 ÷ 365. The Idle Vault uses finer dated periods and compounding.",
      fields: [{ label: "Balance", value: primarySavings?.balance ?? 0, source: primarySavings?.name ?? "No savings balance", editable: true }, { label: "Rate", value: savingsRate, source: activePromo ? "Special interest rate" : "Regular interest rate", editable: true }, { label: "Special rate ends", value: primarySavings?.promotionEnd?.slice(0, 10) ?? "Unknown", source: primarySavings?.name ?? "No balance", editable: true }],
    ...
    },
    {
      ...
      explanation: "Monthly interest is applied, then minimums and the editable extra payment are allocated by the selected ordering.", fields: [{ label: "Mapped debts", value: state.debts.length, source: "Debt map", editable: false }, { label: "Extra monthly payment", value: activeScenario?.debtExtraPayment ?? 0, source: state.projections.activeScenario, editable: true }, { label: "Minimum-path interest", value: debtMinimum.interestPaid, source: "Illustration", editable: false }], assumptions: ["Rates and minimums remain unchanged", "No new borrowing"], exclusions: ["Fees", "Temporary promotional rates", "Lender-specific allocation rules"], sourceTitle: "Moneysmart pay off your credit card", sourceUrl: "https://moneysmart.gov.au/credit-cards/pay-off-your-credit-card",
    },
    {
    ...
    {
      ...common, id: "term-maturity", fantasyTitle: "The Sealed Vault", plainTitle: "When a locked deposit ends", question: "What may be available when the locked period ends?", result: term ? `${money(state, termFuture)} estimated at the end using ${term.compounding === "monthly" ? "interest added monthly" : "simple interest"}` : "Add an end date and interest rate to a locked balance",
    ...
    },
    {
      ...
      explanation: "Each month, interest is added and the chosen monthly contribution is included at the end of the month.", fields: [{ label: "Starting amount", value: compoundPrincipal, source: primarySavings?.name ?? "No savings balance", editable: true }, { label: "Yearly interest rate", value: compoundRate, source: primarySavings?.name ?? "Unknown", editable: true }, { label: "Monthly contribution", value: compoundContribution, source: state.projections.activeScenario, editable: true }, { label: "Years", value: compoundYears, source: "You can change this", editable: true }], assumptions: ["The interest rate and contribution stay the same", "Interest is added monthly and contributions arrive at month end"], exclusions: ["Tax", "Fees", "Rate changes", "Missed contributions"], sourceTitle: "Moneysmart compound interest calculator", sourceUrl: "https://moneysmart.gov.au/saving/compound-interest",
    },
    {
